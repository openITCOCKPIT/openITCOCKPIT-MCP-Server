...
**Response shape.** Tools that return a list answer with an envelope, never a
bare array:
...
```json
{
  ...
  "count": 50,
  "truncated": true,
  "hint": "More rows exist than the 50 returned. ... narrow the query using name_filter"
}
```

**Missing arguments.** `hostname` and `servicename` are required where they
appear and have no estate-wide form. A call that omits one is answered with the
values that would have worked - for `hostname`, the instance's actual host
names - so the caller can correct it instead of retrying.

`truncated` matters: openITCOCKPIT caps its list endpoints and reports no total,
so without it a partial answer is indistinguishable from a complete one. Every
list tool takes `limit` (default 50, max 500).

A result is delivered twice: as JSON text in `content`, and as
`structuredContent` for clients on MCP revision 2025-06-18 or later. Setting
`OITC_COMPACT_CONTENT=true` reduces `content` to a one-line summary and roughly
halves each response - but then **only clients that read `structuredContent` see
the data**. Leave it off for Open WebUI and anything else that reads `content`
only.
...
clients they are plain Markdown - paste what you need. See
...
pip install -e ".[dev]"
pytest                              # 189 tests
pytest --cov=openitcockpit_mcp      # coverage report
ruff check .
mypy
```

`tests/test_tools_smoke.py` runs every tool once against stubbed openITCOCKPIT
responses, so a new tool has to be added to its call table.

### Layout

```
src/openitcockpit_mcp/
  config.py       Settings from env and .env, validated, no import-time I/O
  auth.py         Bearer-token verification for the http transport
  client.py       OITCClient: the openITCOCKPIT HTTP API, URL-encoded, TLS-verified
  errors.py       HTTP responses -> errors an agent can act on
  resolvers.py    Human-readable names -> internal ids
  ...
  tools/write/    Write tools, registered only when enabled
  server.py       Assembly
  cli.py          oitc-mcp entrypoint
docs/
  write-tools.md            Scope rules and read-modify-write semantics
  openitcockpit-api-notes.md  API behaviour this server works around
skills/
  system-prompt.md + oitc-*/SKILL.md   Tool-chaining recipes for LLM clients
```
